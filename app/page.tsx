...
import { ArrowRight, Sparkles, Heart, TrendingUp, MapPin, Clock, Zap, Star, Leaf, UtensilsCrossed, Landmark, Trophy, CalendarHeart, BookOpen, Users, CheckCircle2, Shield, Send, Mail } from "lucide-react"
...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* ── FOCUS AREAS ─── */}
      <section className="px-4 py-16 md:px-6 md:py-24 bg-latte/30">
        <div className="mx-auto max-w-6xl">
          <SlideUp>
      ...
      <section className="relative px-4 py-16 md:px-6 md:py-24">
        <div className="relative z-10 mx-auto max-w-6xl">
          <SlideUp>
            <div className="mb-12 flex flex-col items-center text-center">
              ...
              <p className="mt-3 max-w-lg text-sm text-espresso/45">
                {"Real gigs from real orgs. One-tap apply, show up, make an impact."}
              </p>
            </div>
          </SlideUp>
          ...
          <StaggerChildren className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {featured.map((op) => (
              <StaggerItem key={op.id} className="flex">
      ...
      <section className="px-4 py-16 md:px-6 md:py-24 bg-latte/40">
        <div className="mx-auto max-w-6xl">
          ...
          <StaggerChildren className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[
              { org: "Nature Vancouver", program: "New Brighton Park Restoration", abbr: "NV", color: "bg-matcha/12", textColor: "text-matcha-dark", borderColor: "border-matcha/25" },
              { org: "DIVERSECity", program: "Youth for People and the Planet", abbr: "DC", color: "bg-sky/12", textColor: "text-sky-dark", borderColor: "border-sky/25" },
              { org: "Apathy is Boring", program: "RISE", abbr: "AB", color: "bg-honey/12", textColor: "text-espresso/70", borderColor: "border-honey/25" },
              { org: "CityHive", program: "Youth Climate Action", abbr: "CH", color: "bg-rose/10", textColor: "text-rose", borderColor: "border-rose/20" },
            ].map((partner) => (
              <StaggerItem key={partner.org}>
                <motion.div whileHover={{ y: -4 }} transition={{ type: "spring", stiffness: 300 }}>
                  <Card className={cn(cardClass, "h-full border-2", partner.borderColor)}>
                    <CardContent className="p-5 flex flex-col items-center text-center gap-3">
                      <div className={cn("flex h-14 w-14 items-center justify-center rounded-2xl text-lg font-extrabold", partner.color, partner.textColor)}>
                        {partner.abbr}
                      </div>
                      <div>
                        <h3 className="text-sm font-bold text-espresso">{partner.org}</h3>
                        <p className="text-[11px] text-espresso/40 mt-1">{partner.program}</p>
                      </div>
                    </CardContent>
                  </Card>
                </motion.div>
      ...
      {/* ── ACTIVE PILOT + WAITLIST/FEEDBACK ─── */}
      <section className="relative px-4 py-16 md:px-6 md:py-24">
        <div className="mx-auto max-w-6xl">
            ...
            <SlideUp>
              <div>
                ...
                <p className="text-sm text-espresso/50 leading-relaxed mb-3">
                  {"Something is a live pilot -- we're actively building and iterating based on what our users actually want. This platform is co-creative: your feedback shapes every feature, every flow, every decision."}
                </p>
                <p className="text-sm text-espresso/50 leading-relaxed mb-6">
                  {"Right now we're focused on Metro Vancouver, connecting youth volunteers with environmental, civic, and food organizations. We're small on purpose, and growing with intention."}
                </p>

                <Card className={cn(cardClass, "overflow-hidden mb-4")}>
                  <div className="h-1.5 bg-gradient-to-r from-matcha/40 via-sky/30 to-honey/30" />
                  <CardContent className="p-5">
                    <div className="flex items-center gap-4 mb-4">
                      <div className="relative h-14 w-14 rounded-2xl overflow-hidden bg-matcha/10 shrink-0">
                        <Image src="/images/cathy-luo.jpg" alt="Cathy Luo, founder of Something" fill className="object-cover" />
                      </div>
                      <div>
                        <h3 className="text-sm font-bold text-espresso">Cathy Luo</h3>
                        <p className="text-[11px] text-espresso/40">Founder & Lead, Something</p>
                      </div>
                    </div>
                    <p className="text-xs text-espresso/50 leading-relaxed italic">
                      {"\"I built Something because I was tired of volunteering feeling like a chore. It should feel like showing up for your people -- because that's exactly what it is.\""}
                    </p>
                  </CardContent>
                </Card>
          ...
          <div className="grid gap-8 lg:grid-cols-2 items-start">
            ...
            <SlideUp delay={0.1}>
              <div className="flex flex-col gap-3">
                {[
                  { title: "Volunteer Buddies & Crews", description: "Get matched with a volunteer buddy or join a regular crew for ongoing projects. Never show up alone.", icon: Users, tag: "Community" },
...
function WaitlistForm() {
  ...
  return (
    <Card className={cn(cardClass, "overflow-hidden")}>
      <div className="h-1.5 bg-gradient-to-r from-matcha/50 to-matcha/20" />
      <CardContent className="p-5">
        <div className="flex items-center gap-2 mb-3">
          <Mail className="h-4 w-4 text-matcha-dark" />
          <h3 className="text-sm font-bold text-espresso">Join the waitlist</h3>
        </div>
        <p className="text-xs text-espresso/45 mb-4">
          {"Get notified when we launch new features or expand to your area."}
        </p>
        <form onSubmit={handleSubmit} className="flex gap-2">
          <Input
            ...
            className="flex-1 rounded-full border-border/60 bg-latte/50 text-sm"
          ...
          <ScaleOnTap>
            <Button
              type="submit"
              disabled={status === "loading" || status === "success"}
              className="rounded-full bg-matcha text-espresso hover:bg-matcha-dark font-bold text-sm px-5"
            >
        ...
        <AnimatePresence>
          {status === "success" && (
            <motion.p
              ...
              exit={{ opacity: 0 }}
              className="text-xs text-matcha-dark mt-2 flex items-center gap-1"
            >
              <CheckCircle2 className="h-3 w-3" /> {"You're on the list!"}
            </motion.p>
          )}
          {status === "error" && (
            <motion.p
...
function FeedbackForm() {
  ...
  return (
    <Card className={cn(cardClass, "overflow-hidden")}>
      <div className="h-1.5 bg-gradient-to-r from-sky/50 to-sky/20" />
      <CardContent className="p-5">
        <div className="flex items-center gap-2 mb-3">
          <Send className="h-4 w-4 text-sky-dark" />
          <h3 className="text-sm font-bold text-espresso">Share a suggestion</h3>
        </div>
        <p className="text-xs text-espresso/45 mb-4">
          {"Have an idea or question? We read everything."}
        </p>
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          <Textarea
            ...
            className="rounded-xl border-border/60 bg-latte/50 text-sm min-h-[80px] resize-none"
            disabled={status === "loading" || status === "success"}
          />
          <ScaleOnTap className="self-end">
            <Button
              ...
              className="rounded-full bg-sky text-espresso hover:bg-sky-dark font-bold text-sm px-5"
            >
        ...
        <AnimatePresence>
          {status === "success" && (
            <motion.p
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0 }}
              className="text-xs text-sky-dark mt-2 flex items-center gap-1"
            >
              <CheckCircle2 className="h-3 w-3" /> Thanks for your feedback!
            </motion.p>
          )}
        </AnimatePresence>
      </CardContent>
